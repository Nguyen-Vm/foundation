...
import org.linker.foundation.utils.ByteUtils;
import org.linker.foundation.utils.CollectionUtils;
import org.linker.foundation.utils.DateUtils;
import org.linker.foundation.utils.GenericUtils;
import org.linker.foundation.utils.JsonUtils;
...
public class RedisPairOperations {
    ...
    public List<String> hKeys(String key) {
        ...
        return byteList.stream().map(k -> new String(k, ByteUtils.UTF8)).collect(Collectors.toList());
    }
    ...
    public <T> T hGetAsBean(String key, Class<T> clazz) {
        ...
        for (Map.Entry<byte[], byte[]> entry : byteMap.entrySet()) {
            String fieldName = new String(entry.getKey(), ByteUtils.UTF8);
            ...
            if (null != field) {
                if (Collection.class.isAssignableFrom(field.getType())) {
                    ...
                    json.put(fieldName, JsonUtils.parseArray(new String(entry.getValue(), ByteUtils.UTF8), fClass));
                } else {
    ...
    /** 判断 Redis HashMap 存储的 field QL 值 */
    ...
    /** Redis HashMap 存储 field QL, 仅当 field 不存在时 */
    ...
    /** Redis HashMap 存储 field QL 带过期时间 */
